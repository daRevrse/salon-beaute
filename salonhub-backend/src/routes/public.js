...
router.get('/salon/:slug/availability', async (req, res) => {
  try {
    ...
    console.log('Debug - businessHours:', JSON.stringify(businessHours, null, 2));
...
router.post('/appointments', async (req, res) => {
  try {
    ...
    if (!salon_slug || !first_name || !last_name || !phone || !service_id || !appointment_date || !start_time) {
    ...
    const conflicts = await db.query(
      ...
      [tenantId, appointment_date, start_time, start_time, end_time, end_time, start_time, end_time]
    );
    ...
    if (client.length > 0) {
      ...
      await db.query(
        ...
        [first_name, last_name, email, preferred_contact_method || 'email', clientId, tenantId]
      );
    } else {
      ...
      const result = await db.query(
        `INSERT INTO clients (tenant_id, first_name, last_name, email, phone, preferred_contact_method, created_at)
         VALUES (?, ?, ?, ?, ?, ?, NOW())`,
        [tenantId, first_name, last_name, email, phone, preferred_contact_method || 'email']
      );
    ...
    const appointment = await db.query(
      ...
      [tenantId, clientId, service_id, appointment_date, start_time, end_time, notes || null]
    );
    ...
    res.status(201).json({
      ...
      message: 'Votre rendez-vous a été enregistré avec succès. Vous recevrez une confirmation prochainement.'
    });
